...
class InvoiceDetailImport implements ToModel, WithHeadingRow
{
    public function model(array $row)
    {
        ...
        DB::transaction(function () use ($row) {
            ...
            // 2. Retrieve patient_id
            $patient = PatientDetail::where('phone_number', $row['patient_mobile'])->first();
            if (!$patient) {
            }
            ...
            END, expiry_date ASC') // Handle stocks with NULL expiry dates first
                    ...
                    if ($remainingQuantity > 0) {
                        ...
                        if ($fallbackStock) {
                            ...
                            StockTransaction::create([
                                'stock_id' => $fallbackStock->id,
                                'item_id' => $item->id,
                                'invoice_id' => $invoiceDetail->id,
                                'quantity' => $remainingQuantity,
                                'item_price' => $fallbackStock->item_price,
                                'total_price' => $remainingQuantity * $fallbackStock->item_price,
                                'status' => 2, // Outgoing stock
                                'created_at' => now(),
                            ]);
                        }
